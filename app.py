...
openai.api_key = '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'
...
def Alumni_chat():
    if request.method == "POST":
        ...
        print("User Question", question)
        print(request.args)
        alumni_name = input_info[0]['Alumni Name']  # Default alumni name
        graduation_year = input_info[1]['Graduation Year']  # Graduation year
        # question = "What's your experience at our university?"  # Default question

        # Create or load the context for the conversation
        context = [{'role': 'system',
                    'content': f"{alumni_name} graduated in {graduation_year}. They can provide insights and advice related to their experience at our university and their current work field. Please feel free to ask any questions."}]
        ...
        print(type(response))
        print("Done")
        # return render_template('alumni.html', alumni_name=alumni_name, user_message=question, assistant_response=response)
        return jsonify({"alumni_name": alumni_name, "user_message": question, "assistant_response": response})
